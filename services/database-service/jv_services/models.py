...
class JVBatchJob(models.Model):
    ...
    class Meta:
        db_table = "jv_batch_jobs"
        ordering = ["-created_at"]


class JVBatchJobItem(models.Model):
    class Status(models.TextChoices):
        PENDING = "pending", "Pending"
        APPLIED = "applied", "Applied"
        FAILED = "failed", "Failed"
    ...
    job = models.ForeignKey(JVBatchJob, on_delete=models.CASCADE, related_name="items")

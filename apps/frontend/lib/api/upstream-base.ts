...
function normalizeOriginBase(value: string | undefined): string | null {
  const normalized = normalizeBase(value);
  if (!normalized) {
    return null;
  }
...
export function resolveBackendApiBaseCandidates(env: RuntimeEnv = process.env): string[] {
  return uniqueCandidates([
    normalizeOriginBase(env.BACKEND_INTERNAL_API_BASE_URL),
    normalizeOriginBase(env.BACKEND_ORIGIN),
    normalizeOriginBase(env.BACKEND_API_BASE_URL),

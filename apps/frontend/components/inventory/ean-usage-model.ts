export type EanUsageTimelineRow = {
  id: string;
  event: string;
  status: string;
  marketplace: string;
...
export function buildEanUsageTimelineRows(usages: unknown[]): EanUsageTimelineRow[] {
  return usages
    .map((item, index) => {
    ...
    .filter((item): item is EanUsageTimelineRow => item !== null)

...
function extractSiteEans(item: Record<string, unknown>, fallback: string) {
  return {
    ...
    ebayDep: normalizeEanValue(item.ebay_dep_ean ?? item.ebay_ean_dep ?? item.ean_ebay_dep ?? item.ebayDepEan, fallback),
  ...
  useEffect(() => {
    ...
    const mappedRows = items.map((item) => {
      ...
      const normalizedSiteEanStatuses = {
        ...
        ebayDep: typeof rawStatus.ebay_dep === "boolean" ? rawStatus.ebay_dep : null,

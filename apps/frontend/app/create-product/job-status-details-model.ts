...
function toObject(value: string): Record<string, unknown> | null {
  if (!value.trim()) {
    return null;
  }
  try {
...
function toCount(payload: Record<string, unknown> | null): number {
  if (!payload) return 0;
...
export function buildJobStatusDetails(input: {
  ...
  const jobIdRaw = statusPayload?.job_id ?? statusPayload?.id ?? input.latestJobId;
  ...
  return {
    jobId: typeof jobIdRaw === "string" ? jobIdRaw : input.latestJobId,
